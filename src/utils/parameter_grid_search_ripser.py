"""
Parameter Grid Search for Topological Data Analysis using Ripser

This script performs systematic parameter optimization for k-neighbors and max_edge_length
in persistent homology computation using Ripser for improved performance. It uses synthetic 
datasets from dataset.py and distance computation functions to explore the parameter space 
comprehensively.

The Ripser implementation typically offers 5-100x speedup over Gudhi, enabling more
extensive parameter searches and larger datasets.

Usage:
    python src/utils/parameter_grid_search_ripser.py [--config path/to/search_config.yaml]
"""
...
class ParameterGridSearchRipser:
    """
    Main class for parameter grid search optimization using Ripser.
    
    Systematically explores k-neighbors and max_edge_length parameter space
    to find optimal values for topological analysis, with significantly improved
    performance compared to Gudhi-based implementation.
    """
    ...
    def setup_logging(self):
        ...
        # Create logger
        self.logger = logging.getLogger('parameter_search_ripser')
        ...
        if self.config['logging']['log_to_file']:
            log_file = self.output_dir / 'logs' / 'parameter_search_ripser.log'
    ...
    def compute_persistent_homology(self, distance_matrix: np.ndarray, 
        ...
        """
        Compute persistent homology using Ripser for improved performance.
        ...
        try:
            ...
            result = ripser(distance_matrix.astype(np.float64),
            ...
            # Compute Betti numbers by counting features that persist at max_edge_length
            epsilon = 1e-10
            betti_numbers = []
            ...
            for dim in range(max_dimension + 1):
                if dim < len(diagrams):
                    ...
                    if len(diagram) > 0:
                        births = diagram[:, 0]
                        deaths = diagram[:, 1]
                        # Count features that persist at max_edge_length
            ...
            # Estimate number of simplices (Ripser doesn't directly provide this)
            # We'll use the number of persistence pairs as a proxy
            num_simplices_estimate = sum(len(dgm) for dgm in diagrams)
            
            return {
                'betti_numbers': betti_numbers,
                'num_simplices': num_simplices_estimate,
                'computation_time_ripser': ripser_time,
                'diagrams': diagrams,
                'success': True
            }
    ...
    def run_single_parameter_combination(self, k: int, max_edge_length: float, 
        ...
        try:
            ...
            results = {
                ...
                'computation_time_ripser': homology_results['computation_time_ripser'],
                'diagrams': homology_results['diagrams'],
    ...
    def run_grid_search(self) -> Dict[str, Any]:
        ...
        """
        self.logger.info("Starting parameter grid search with Ripser...")
        self.logger.info("Note: Ripser typically provides 5-100x speedup over Gudhi")
        ...
        for k in k_values:
            for edge_length in edge_lengths:
                ...
                for dataset_idx in range(len(self.datasets)):
                    ...
                    combination_count += 1
                    total_ripser_time += result.get('computation_time_ripser', 0)
                    ...
                    if self.config['performance']['show_progress'] and \
                       combination_count % self.config['performance']['progress_frequency'] == 0:
                        ...
                        avg_ripser_time = total_ripser_time / combination_count
                        self.logger.info(f"Progress: {progress:.1f}% ({combination_count}/{total_combinations}) - Avg Ripser time: {avg_ripser_time:.3f}s")
        ...
        self.results = {
            ...
            'implementation': 'ripser',
            'total_ripser_time': total_ripser_time,
            'avg_ripser_time': total_ripser_time / len(all_results) if all_results else 0
        }
        
        self.logger.info(f"Grid search completed successfully!")
        self.logger.info(f"Total Ripser computation time: {total_ripser_time:.2f}s")
        self.logger.info(f"Average Ripser time per computation: {self.results['avg_ripser_time']:.3f}s")
        
        return self.results
    
    def save_checkpoint(self, results: List[Dict]) -> None:
        """Save intermediate results as checkpoint."""
        checkpoint_file = self.output_dir / 'checkpoints' / f'checkpoint_ripser_{len(results)}.pkl'
        ...
        for k in k_values:
            for edge_length in edge_lengths:
                ...
                summary_stats[key] = {
                    ...
                    'ripser_time_mean': np.mean([r['computation_time_ripser'] for r in param_results]),
                    'ripser_time_std': np.std([r['computation_time_ripser'] for r in param_results]),
    ...
    def create_betti_heatmaps(self) -> None:
        ...
        for dim in range(max_dimension + 1):
            ...
            axes[dim].set_title(f'H{dim} (Betti {dim}) - Ripser')
        ...
        if self.config['output']['save_individual_plots']:
            filename = self.output_dir / 'plots' / 'betti_heatmaps_ripser.png'
    ...
    def create_stability_plots(self) -> None:
        ...
        for dim in range(max_dimension + 1):
            ...
            axes[dim].set_title(f'H{dim} Stability (Std Dev) - Ripser')
            axes[dim].set_xlabel('k-neighbors')
            axes[dim].set_ylabel('Max Edge Length')
        ...
        # Save plot
        if self.config['output']['save_individual_plots']:
            filename = self.output_dir / 'plots' / 'stability_plots_ripser.png'
            plt.savefig(filename, dpi=self.config['visualization']['dpi'], 
                       bbox_inches='tight')
    ...
    def create_connectivity_plots(self) -> None:
        ...
        ax.set_title('Graph Connectivity Ratio - Ripser')
        ...
        # Save plot
        if self.config['output']['save_individual_plots']:
            filename = self.output_dir / 'plots' / 'connectivity_plots_ripser.png'
            plt.savefig(filename, dpi=self.config['visualization']['dpi'], 
                       bbox_inches='tight')
    ...
    def create_sensitivity_plots(self) -> None:
        ...
        for dim in range(max_dimension + 1):
            ...
            ax.set_title(f'H{dim} vs k-neighbors (Ripser)')
            ax.legend()
        ...
        for dim in range(max_dimension + 1):
            ...
            ax.set_ylabel(f'H{dim} (Betti {dim})')
            ax.set_title(f'H{dim} vs Edge Length (Ripser)')
            ax.legend()
            ax.grid(True, alpha=0.3)
        ...
        # Save plot
        if self.config['output']['save_individual_plots']:
            filename = self.output_dir / 'plots' / 'sensitivity_plots_ripser.png'
            plt.savefig(filename, dpi=self.config['visualization']['dpi'], 
                       bbox_inches='tight')
    ...
    def create_timing_plots(self) -> None:
        """Create computational timing analysis plots with Ripser-specific metrics."""
        summary_stats = self.results['summary_stats']
        k_values = self.results['k_values']
        edge_lengths = self.results['edge_lengths']
        ...
        # Ripser-specific timing matrix
        ripser_timing_matrix = np.zeros((len(edge_lengths), len(k_values)))
        ...
        for i, edge_length in enumerate(edge_lengths):
            for j, k in enumerate(k_values):
                ...
                if key in summary_stats:
                    ...
                    ripser_timing_matrix[i, j] = summary_stats[key]['ripser_time_mean']
                else:
                    timing_matrix[i, j] = np.nan
                    ripser_timing_matrix[i, j] = np.nan
        
        # Plot 1: Total computation time
        im1 = ax1.imshow(timing_matrix, aspect='auto', cmap='YlOrRd', origin='lower')
        ax1.set_title('Total Computation Time (seconds)')
        ax1.set_xlabel('k-neighbors')
        ax1.set_ylabel('Max Edge Length')
        
        # Set ticks
        ax1.set_xticks(range(len(k_values)))
        ax1.set_xticklabels(k_values)
        ax1.set_yticks(range(len(edge_lengths)))
        ax1.set_yticklabels(edge_lengths)
        
        # Add value annotations
        for i in range(len(edge_lengths)):
            for j in range(len(k_values)):
                if not np.isnan(timing_matrix[i, j]):
                    text = ax1.text(j, i, f'{timing_matrix[i, j]:.1f}s',
                                   ha="center", va="center", color="black", fontsize=8)
        
        plt.colorbar(im1, ax=ax1)
        
        # Plot 2: Ripser-specific computation time
        im2 = ax2.imshow(ripser_timing_matrix, aspect='auto', cmap='YlGnBu', origin='lower')
        ax2.set_title('Ripser Computation Time (seconds)')
        ax2.set_xlabel('k-neighbors')
        ax2.set_ylabel('Max Edge Length')
        
        # Set ticks
        ax2.set_xticks(range(len(k_values)))
        ax2.set_xticklabels(k_values)
        ax2.set_yticks(range(len(edge_lengths)))
        ax2.set_yticklabels(edge_lengths)
        
        # Add value annotations
        for i in range(len(edge_lengths)):
            for j in range(len(k_values)):
                if not np.isnan(ripser_timing_matrix[i, j]):
                    text = ax2.text(j, i, f'{ripser_timing_matrix[i, j]:.2f}s',
                                   ha="center", va="center", color="black", fontsize=8)
        ...
        # Save plot
        if self.config['output']['save_individual_plots']:
            filename = self.output_dir / 'plots' / 'timing_plots_ripser.png'
            plt.savefig(filename, dpi=self.config['visualization']['dpi'], 
                       bbox_inches='tight')
    ...
    def create_combined_report(self) -> None:
        ...
        pdf_filename = self.output_dir / 'parameter_search_report_ripser.pdf'
        
        with PdfPages(pdf_filename) as pdf:
            # Add a title page
            fig = plt.figure(figsize=(8.5, 11))
            fig.text(0.5, 0.9, 'Parameter Grid Search Report', 
                    ha='center', va='top', fontsize=24, fontweight='bold')
            fig.text(0.5, 0.85, 'Using Ripser for Fast Persistent Homology', 
                    ha='center', va='top', fontsize=16)
            fig.text(0.5, 0.8, f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}', 
                    ha='center', va='top', fontsize=12)
            
            # Add performance summary
            if 'avg_ripser_time' in self.results:
                fig.text(0.5, 0.7, f'Average Ripser Computation Time: {self.results["avg_ripser_time"]:.3f}s', 
    ...
    def generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for key, stats in summary_stats.items():
            ...
            # Efficiency component (faster is better, emphasize Ripser speed)
            efficiency_score = 1.0 / (1.0 + stats['ripser_time_mean'])
    ...
    def _generate_justification(self, stats: Dict, stability_score: float, 
        ...
        if efficiency_score > 0.5:
            justifications.append(f"fast computation ({stats['ripser_time_mean']:.3f}s with Ripser)")
    ...
    def save_results(self) -> None:
        ...
        if self.config['output']['save_raw_results']:
            if self.config['output']['output_format'] == 'pytorch':
                filename = self.output_dir / 'data' / 'raw_results_ripser.pt'
                torch.save(self.results, filename)
            elif self.config['output']['output_format'] == 'numpy':
                filename = self.output_dir / 'data' / 'raw_results_ripser.npz'
                # Remove non-numpy compatible fields
                results_to_save = {k: v for k, v in self.results.items() 
                                 if k not in ['raw_results', 'summary_stats', 'recommendations', 'config']}
                np.savez_compressed(filename, **results_to_save)
            else:  # csv
                # Convert to DataFrame and save
                df = pd.DataFrame(self.results['raw_results'])
                # Remove complex fields for CSV
                df = df.drop(columns=['diagrams', 'connectivity'], errors='ignore')
                filename = self.output_dir / 'data' / 'raw_results_ripser.csv'
        ...
        if self.config['output']['save_summary_stats'] and 'summary_stats' in self.results:
            filename = self.output_dir / 'data' / 'summary_statistics_ripser.pkl'
        ...
        if self.config['output']['save_recommendations'] and 'recommendations' in self.results:
            filename = self.output_dir / 'data' / 'recommendations_ripser.yaml'
            ...
            for rec in self.results['recommendations']:
                rec_clean = {
                    ...
                    'ripser_time': float(rec['stats']['ripser_time_mean'])
                }
    ...
    def run_complete_analysis(self) -> Dict[str, Any]:
        ...
        """
        start_time = time.time()
        self.logger.info("Starting complete parameter grid search analysis with Ripser")
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "="*60)
        print("PARAMETER GRID SEARCH SUMMARY (RIPSER)")
        print("="*60)
        
        if 'avg_ripser_time' in self.results:
            print(f"\nPerformance Summary:")
            print(f"Average Ripser computation time: {self.results['avg_ripser_time']:.3f}s")
            print(f"Total Ripser computation time: {self.results['total_ripser_time']:.2f}s")
        ...
        if 'recommendations' in self.results:
            ...
            for i, rec in enumerate(self.results['recommendations'][:3], 1):
                ...
                print(f"   Total computation time: {stats['computation_time_mean']:.2f}s")
                print(f"   Ripser computation time: {stats['ripser_time_mean']:.3f}s")
        
        print(f"\nResults saved to: {self.output_dir}")
        print("="*60)


def compare_with_gudhi(config_path: str = 'configs/search_config.yaml'):
    """
    Run a quick comparison between Ripser and Gudhi implementations.
    """
    print("\nRunning performance comparison between Ripser and Gudhi...")
    
    # Create both searchers
    ripser_search = ParameterGridSearchRipser(config_path)
    
    # Generate a small test dataset
    ripser_search.config['data']['num_instances'] = 1
    ripser_search.config['grid_search']['k_neighbors']['values'] = [10, 15]
    ripser_search.config['grid_search']['max_edge_length']['values'] = [2, 3]
    
    ripser_search.generate_datasets()
    
    if ripser_search.datasets:
        # Test one parameter combination
        X = ripser_search.datasets[0]
        k = 10
        max_edge_length = 2.0
        
        # Time Ripser
        start_time = time.time()
        distance_matrix = ripser_search.compute_knn_geodesic_distance(X, k)
        ripser_result = ripser_search.compute_persistent_homology(distance_matrix, max_edge_length)
        ripser_time = time.time() - start_time
        
        # Try to time Gudhi
        try:
            from src.utils.parameter_grid_search import ParameterGridSearch
            gudhi_search = ParameterGridSearch(config_path)
            gudhi_search.datasets = ripser_search.datasets
            
            start_time = time.time()
            gudhi_result = gudhi_search.compute_persistent_homology(distance_matrix, max_edge_length)
            gudhi_time = time.time() - start_time
            
            print(f"\nComparison Results:")
            print(f"Dataset size: {X.shape}")
            print(f"Parameters: k={k}, max_edge_length={max_edge_length}")
            print(f"\nRipser:")
            print(f"  Time: {ripser_time:.3f}s")
            print(f"  Betti numbers: {ripser_result['betti_numbers']}")
            print(f"\nGudhi:")
            print(f"  Time: {gudhi_time:.3f}s")
            print(f"  Betti numbers: {gudhi_result['betti_numbers']}")
            print(f"\nSpeedup: {gudhi_time/ripser_time:.1f}x")
            
        except ImportError:
            print("Could not import Gudhi version for comparison")
            print(f"Ripser time: {ripser_time:.3f}s")
            print(f"Ripser Betti numbers: {ripser_result['betti_numbers']}")


def main():
    """Main function for command-line usage."""
    parser = argparse.ArgumentParser(description="Parameter Grid Search for Topological Data Analysis using Ripser")
    parser.add_argument('--config', type=str, default='configs/search_config.yaml',
                       help='Path to configuration file')
    parser.add_argument('--output', type=str, default=None,
                       help='Override output directory')
    parser.add_argument('--compare', action='store_true',
                       help='Run comparison with Gudhi implementation')
    
    args = parser.parse_args()
    
    # Run comparison if requested
    if args.compare:
        compare_with_gudhi(args.config)
        return
    
    # Create grid search instance
    search = ParameterGridSearchRipser(config_path=args.config)
